#!/usr/bin/env bash
# Shared settings and helpers for the Nametag Studio scripts.
...
# 16MB flash, OPI PSRAM, USB-CDC serial, and a large app partition because the
# PNG + JPEG decoders and the graphics library don't fit the default scheme.
...
detect_port() {
  if [ -n "${NAMETAG_PORT:-}" ]; then
    echo "$NAMETAG_PORT"; return 0
  fi
